...
class HexTest: public ::testing::Test
    ...
    TEST(HexTesting, HexAdd) {
        ...
        Hex j("2");
        EXPECT_THROW(Hex m = hexmath::Add(i, j), std::overflow_error);
    }
    ...
    TEST(HexTesting, HexBitShift) {
        Hex a("A0");
        EXPECT_TRUE(hexmath::Equals(a.BitShiftLeft(2), Hex("A000")));
        Hex b("1245");
        EXPECT_TRUE(hexmath::Equals(b.BitShiftRight(2),Hex("12")));
        Hex c("-2451");
        EXPECT_TRUE(hexmath::Equals(c.BitShiftLeft(1), Hex("-24510")));
        Hex d("-8902");
        EXPECT_TRUE(hexmath::Equals(d.BitShiftRight(1), Hex("-890")));
        EXPECT_NO_THROW(d.BitShiftLeft(-23));
        EXPECT_NO_THROW(d.BitShiftRight(-100));
    }
 }
